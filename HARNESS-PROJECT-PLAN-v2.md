...
- Chỉ hỗ trợ single-task execution. Concurrent tasks sẽ được hỗ trợ ở Phase 3.
...
| AC-07 | Mọi executable step phải có rollback capability | Checkpoint trước mỗi step | — |
...
Knowledge sources mặc định:

- `docs/` — architecture, conventions, ADR, glossary
- `README.md` — project overview
- Paths bổ sung có thể cấu hình qua `project.yaml` → `knowledge.include`
...
Verification không gọi ngược Runtime.

Planning và Runtime truy cập AI thông qua AI Adapter. Không module nào gọi AI API trực tiếp.

Mỗi module chỉ giao tiếp với module ở tầng liền kề hoặc AI Adapter khi cần thiết.
...
# Optional overrides
context:
  budget_tokens: 50000       # default 50,000

knowledge:
  include:                    # paths bổ sung ngoài defaults (docs/, README.md)
    - CHANGELOG.md
    - CONTRIBUTING.md

verification:
  auto_approve_risk: LOW      # LOW | MEDIUM — risk cao hơn luôn cần human
...
[7] Approval Gate
    LOW      → auto approve (nếu project.yaml cho phép)
    MEDIUM   → notify, auto approve sau 30 phút nếu không phản hồi
    HIGH     → human approval bắt buộc
    CRITICAL → senior + second opinion
```
...
> Chi tiết retry strategy, rollback, scope enforcement: xem [TECHNICAL_DESIGN.md](./TECHNICAL_DESIGN.md)
...
| ADR     | Nội dung                                 |
| ------- | ---------------------------------------- |
| ADR-001 | Repository là Source of Truth            |
| ADR-002 | Runtime ngoài Repository                 |
| ADR-003 | Plan before Execute                      |
| ADR-004 | Deterministic Risk Scoring               |
| ADR-005 | Knowledge Engine là entry point duy nhất |
| ADR-006 | AI Adapter abstraction                   |
| ADR-007 | Verification by Risk                     |
| ADR-008 | Learning from Failure                    |
| ADR-009 | Verification độc lập AI                  |
...
- [ ] Architecture Constraints (AC-01 đến AC-08) đã được team đọc và không có phản đối
