...
| `harness-workflow` | Quy trình Harness | Vòng đời session (CTR gate, RIPER-5, artifacts) |
| `strategic-compact` | Quản lý Context Chiến lược | Quản lý dung lượng context window một cách hiệu quả |
| `csharp-baseline` | C# Baseline | C# stack baseline (architecture, naming, dependencies) — auto khi dotnet |
| `php-baseline` | PHP Baseline | PHP conventions (Composer, PSR, XAMPP) — auto khi php |
...
| `subagent-driven-development` | Phát triển qua Subagent | Điều phối và ủy thác công việc cho các agent con (bao gồm DAG decomposition) | subagent, parallel, worker, delegate, dispatch, dag, fan-out, stage, phân công, ủy thác, song song |
...
| Skill | Tiếng Việt | Mục đích | Keywords |
|-------|-----------|----------|----------|
| `csharp-bugfix` | Fix Bug C# | Quy trình fix bug trong C# / ABP (behavior/logic bugs) | csharp, dotnet, bug, fix, exception, null, ef-core, c#, .net, ngoại lệ, null |
...
- **Tier 1 (Core):** Luôn gợi ý 5 skill cốt lõi (`karpathy-guidelines`, `harness-workflow`, `strategic-compact` + stack baseline `csharp-baseline`/`php-baseline` khi đúng stack)
