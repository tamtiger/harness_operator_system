...
| `task_type` | string | ❌ | Phân loại loại tác vụ (ví dụ: `'feature'`, `'bugfix'`, `'refactor'`, `'test'`, `'docs'`, `'config'`, `'research'`, `'debug'`, `'hotfix'`) |

```json
// Response
{ "id": "t-7f8a9b0c", "title": "...", "status": "pending", "scope": "...", "task_type": "feature" }
...
Nâng cấp instinct thông qua Quy trình thăng cấp (Promotion Pipeline). Trạng thái của instinct sẽ chuyển thành `candidate` và chạy qua cổng kiểm soát Regression Gate. Nếu vượt qua cổng kiểm soát, instinct sẽ được đưa vào giai đoạn chạy quan sát (`shadow`) với confidence bắt đầu từ 0.5. Nếu không vượt qua, instinct giữ nguyên trạng thái `candidate` và trả về lỗi.
...
```json
// Response (vượt qua cổng kiểm soát thành công, chuyển sang shadow)
{
  "ok": true,
  "id": "inst-a1b2c3d4",
  "status": "shadow",
...
// Response (không vượt qua cổng kiểm soát, giữ nguyên candidate)
{
  "ok": false,
  "id": "inst-a1b2c3d4",
  "status": "candidate",
  "error": "Failed regression gate: confidence < 0.7",
  "gate_check": {
    "passed": false,
    "failed_checks": ["confidence < 0.7"],
    "reason": "Failed regression gate: confidence < 0.7"
  }
}
...
Phân tích vết thực thi (trace) lịch sử của các tasks và sessions trong kho lưu trữ dữ liệu để phát hiện lỗi lặp, loops, workflow non-compliance (vi phạm quy trình) hoặc forgetting (sự lãng quên).
  ...
  "signals": [
    {
      ...
      "instinct_id": "inst-1234",
      "session_id": "sess-5678",
      "task_id": "task-90ab",
      "payload": {
        "consecutive_failures": 3,
        "task_type": "bugfix",
        "last_failed_at": "2026-06-19T11:00:00Z"
      }
    }
  ],
  "summary": {
    "total_scorecards": 47,
    "signals_found": 1,
    "last_analyzed": "2026-06-19T11:15:00Z"
  }
}
```
...
Gửi đề xuất tối ưu hóa (merge, prune, evolve, penalize) cho các instincts dựa trên các tín hiệu lỗi phát hiện được. Đồng thời chạy thử Regression Gate cảnh báo sớm.

| Parameter | Type | Required | Mô tả |
|-----------|------|----------|-------|
| `repo_path` | string | ✅ | Đường dẫn tới repo |
| `type` | enum | ✅ | Loại đề xuất: `"merge"` \| `"prune"` \| `"evolve"` \| `"penalize"` |
| `instinct_ids` | string[] | ✅ | Danh sách các target instinct IDs |
| `rationale` | string | ✅ | Giải thích logic của đề xuất |
| `suggested_change` | string | ❌ | Nội dung thay đổi chi tiết được đề xuất |

```json
// Response
{
  "proposal_id": "prop-5678abcd",
