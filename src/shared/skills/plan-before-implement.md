...
**Announce at start:** "I'm using the plan-before-implement skill to create the implementation plan."

**Save plans to:** `docs/harness/plans/YYYY-MM-DD-<feature-name>.md`
...
> **For agentic workers:** Use subagent-per-task or executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
2. **Placeholder scan:** Search for red flags from "No Placeholders" section above. Fix them.
...
**"Plan complete and saved to `docs/harness/plans/<filename>.md`. Two execution options:**
...
**If Subagent-Driven chosen:** Use subagent-per-task skill

**If Inline Execution chosen:** Execute tasks inline with verification checkpoints
