...
export async function runProposalReject(id: string, options: any = {}) {
  ...
  try {
    ...
    const comments = options.comments || 'Rejected via CLI';
    const prop = await service.rejectProposal(id, reviewer, comments);
