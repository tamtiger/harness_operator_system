...
export function runOrchestrate(
  ...
  log("info", `Starting Ralph Loop orchestrator for task: "${taskTitle}"`, { maxLoops, repoPath });
  ...
  while (loops < maxLoops && !done) {
    ...
    const verify = verifyRun(repoPath, { steps: options.steps, task_id: taskId });
    ...
    if (verify.passed) {
      ...
      const failedStep = verify.step_results.find(r => !r.passed)?.name || "unknown";
  ...
  sessionHandoff(session.session_id, `Orchestrator failed to complete task "${taskTitle}" after ${loops} loops`, [lastError], ["Investigate verification failure logs"]);
